...
"""
This just contains old ad-hoc methods used to correct for a wrong
usage of the alignment methods, see the doc of 
/home/perezheg/Programs/sofi_functions/sofi_functions/sequence_utils_old.py
for the keyword re_merge_skipped_entries
"""
def alignment_result_to_list_of_dicts(ialg, topology_0,
    ...
    seq_0_res_idxs
    re_merge_skipped_entries: boolean, default is False

        Note
        ====
          Avoid using this parameter when possible. It was an ad-hoc
          correction to :obj:`Bio.pairwise2.align.globalxx` output.
          By using :obj:`Bio.pairwise2.align.globalxs` with arguments
          (seq1,seq2,-1,0), this keyword should not be needed anymore.

          Background:
            :obj:`Bio.pairwis2.align` has different functions for alignment, and
            the "standard", i.e. documented option is :obj:`Bio.pairwise2.align.globalxx`,
            **does not penalize gaps**,so that an alignment
            AAABAAA vs. AAACAAA yields:

            * AAAB~AAA
            * AAA~CAAA

            Setting this boolean to True will try to close the gaps ("~") a posteriori
            so that the alignment results are:

            * AAABAAA
            * AAACAAA

            It is much better practice to use the right, gap-penalising function from
            the beginning, i.e. :obj:`Bio.pairwise2.align.globalxs`
            (and not fuck around with its result a posteriori)

        Warning
        =======
            This keyword and the methods it uses are to be deprecated very soon
