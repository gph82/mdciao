...
def sitefile2site(sitefile):
    with open(sitefile,"r") as f:
        idict = jsonload(f)
    try:
        idict["bonds"]["AAresSeq"]=[item.split("-") for item in idict["bonds"]["AAresSeq"]]
...
if not split_by_neighborhood:
    ...
    for ii, oo in enumerate(final_look):
        ...
        plt.sca(myax[ii])
        label = '%s-%s (%s-%s) (%u)' % (refgeom.top.residue(pair[0]), refgeom.top.residue(pair[1]),
                                        (ctcs_mean[oo] * 100))
        plt.plot(time_array/1e3, actcs[:, oo]*10,
                 label=label)
        plt.legend()
        # plt.yscale('log')
        plt.ylabel('d / nm')
    ...
    for jax, isite in zip(list_of_axes, sites):# in np.unique([ctc_idxs_small[ii] for ii in final_look]):
        ...
        for ii, (iy, ipair) in enumerate(zip(ctcs_mean, isite["res_idxs"])):
            ...
            labels_frags = [in_what_fragment(idx, fragments, fragment_names=fragment_names) for idx in ipair]
            for jj, jcons in enumerate(labels_consensus):
                if str(jcons).lower() != "none":
